...
import "./posts.css";
import { useGetAllBlogs } from "../../hooks/api";
...
const AllPosts = () => {
  ...
  const { isLoading, error, data, refetch } = useGetAllBlogs(
  ...
  return (
    <div className="max-w-[1040px] mt-5 mx-auto h-auto">
      ...
      {error && <h3>error fetching posts</h3>}
      <h3 className="text-xl font-bold uppercase text-orange-500">
        {data?.total} available posts
      </h3>
        ...
        {data &&
          data?.data?.map((item, index) => (
            <div
              ...
              onClick={() => nav(`/single-post/${item?.id}`)}>
              <img
